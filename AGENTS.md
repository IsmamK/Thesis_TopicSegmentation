...
**After every session, every eval run, every bug fix, every result — append a new dated entry to `docs/PROGRESS_LOG.md`.**
Format: `## [YYYY-MM-DD HH:MM] — <headline>` followed by what was done, results (with numbers), and observations.
This is non-negotiable. Do not end a session without updating the log.
