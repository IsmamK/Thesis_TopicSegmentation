...
\section{Prosody and visual integration}

The most immediate extension is the full integration of prosodic features
(pause duration, pitch reset) and visual streams (shot boundaries, slide OCR)
into the RWF fusion, which was implemented but not included in the final
evaluation due to time constraints.
Preliminary experiments suggest an additional \todo{}\% $P_k$ reduction.
A modality-dropout training scheme could make the system robust to missing
modalities (e.g., audio-only or slides-only lectures).
...
with a multilingual text encoder such as LaBSE~\citep{feng2022language} or
\texttt{multilingual-e5-large}; and (2)~using Whisper's multilingual mode for
transcription.
