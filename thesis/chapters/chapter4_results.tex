...
all values are macro averages over the 30 videos. Lower $P_k$ and WindowDiff (WD) are better; these are the primary claim
metrics.
Boundary Similarity (BS) and tolerance-$F_1$ are secondary diagnostics;
they are better when higher but do not drive method ranking.
...
neural alternatives under $P_k$ and WD, which confirms that dense embeddings
are the right foundation for this task. Conservative cross-model boundary
selection then reduces $P_k$ by a further 0.0171 absolute --- the best
...
F1@2, at the cost of $P_k$/WD gains that do not individually reach significance
relative to the cross-model method.
...
BGE-divisive baseline ($P_k=0.3884$). The underlying cause --- a systematic
granularity mismatch between editorial chapters and sub-chapter linguistic
transitions --- is developed in full in the discussion
(Section~\ref{sec:discussion}).

These failures are worth documenting precisely because they are not obvious in advance.
Future projects targeting creator-level chapter segmentation can skip
prosodic and discourse-marker signal integration with this empirical
justification, rather than rediscovering the same result.
...
The diagnostic F1 column in Table~\ref{tab:domain} shows near-zero values in
several domains; this is expected and not a primary weakness.
...
Near-zero exact-match F1 under this operating point indicates that boundaries
...
Taken together, the numbers tell a consistent story. Conservative cross-model
scoring is the most reliable single method we found: it reduces $P_k$ by 0.0171
absolute relative to the BGE-divisive baseline, and that gain holds up after
Holm correction across seven pairwise tests. The balanced method selector
pushes $P_k$ lower still, and it meaningfully improves Boundary Similarity and
F1@2, but its margin over the cross-model method on $P_k$/WD alone falls short
of significance. We therefore report the cross-model result as the main
supported claim and the selector result as strong evidence that per-video method
choice matters --- not as a proof that the selector universally dominates. The
leave-domain-out degradation (selector $P_k=0.4012$ when the held-out domain
is entirely excluded from training) reinforces this: the selector benefits from
having related videos in the training fold, and that condition is not always met
in practice.

\paragraph{The granularity finding.}
The most actionable result from the ablation study is a systematic
granularity mismatch. YouTube chapters represent deliberate, coarse editorial
decisions made by the video creator. Linguistic and acoustic signals ---
discourse markers ($P_k=0.4615$), BERTopic ($P_k=0.5632$), GPT-2 perplexity
($P_k=0.4182$), and pause/pitch transitions ($P_k=0.4174$) --- detect
...
important: it means that slide-change detection is the most promising
...
Within the evaluated framework, candidate selection appears to be the
...
neighbourhood of each chapter transition in most cases but sometimes merging
...
correct major section but might need to scan a minute or two backward or
forward to find the precise transition.
...
To probe whether LecSeg-30 results generalise beyond the benchmark, YouTube
lectures \emph{not} in LecSeg-30 were evaluated using the same BGE-divisive
baseline and cross-model conservative method.
A pool of 20 candidate videos spanning five domains (CS, Mathematics, Physics,
Biology, Philosophy) was assembled from channels not present in LecSeg-30
(3Blue1Brown, Andrej Karpathy, StatQuest, CrashCourse).
...
Videos with fewer than 4 chapters or no available captions were excluded.

Table~\ref{tab:external_eval} summarises results for the videos that passed
the quality filters. Figure~\ref{fig:external_per_video} shows per-video
$P_k$ for both methods relative to LecSeg-30 mean performance.
Figure~\ref{fig:external_per_domain} shows per-domain averages.

\input{tables/external_eval_table}

\IfFileExists{figures/external_eval_per_video.png}{%
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/external_eval_per_video.png}
  \caption{Per-video $P_k$ on external videos. Dashed lines mark the LecSeg-30
    mean for each method; dotted red line marks random baseline ($P_k=0.5$).
    Bars are coloured by domain.}
...
\begin{figure}[t]
  ...
  \caption{Per-domain mean $P_k$ on external videos compared to LecSeg-30 means
    (dashed lines). Each bar pair shows BGE-divisive (solid) and cross-model
    (hatched). Domains with fewer than 2 external videos are shown as single
    data-point estimates.}
  \label{fig:external_per_domain}
\end{figure}
}{}

BGE-divisive transfers consistently across available domains: per-video $P_k$
values broadly track the LecSeg-30 mean ($0.388$), with variance explained
primarily by video length and chapter granularity rather than any systematic
direction.
The cross-model conservative method shows greater external variance,
generally degrading relative to its LecSeg-30 mean ($0.371$).

Two confounds limit interpretation.
First, external captions are yt-dlp auto-subtitles chunked by word count,
whereas LecSeg-30 used faster-Whisper with spaCy sentence splitting;
sentence boundary quality differs systematically.
Second, the external set is small and biased toward 3Blue1Brown-style
explainer videos, which may not represent the full domain diversity of
LecSeg-30.
The results are therefore a qualitative consistency check, not a held-out test.

The BGE-divisive consistency supports the claim that the core text-embedding
signal is not purely an artefact of the Whisper pipeline or the LecSeg-30
creator pool.
The cross-model degradation reinforces the recommendation in
Section~\ref{sec:official_claim}: treat the cross-model result as a strong
LecSeg-30 operating point, not a universally deployable method.
