...
The \lecseg{} pipeline takes a raw lecture video and turns it into chapter-level
segments and, when annotations are available, a two-level chapter/subtopic
hierarchy. It runs in five stages, shown in Figure~\ref{fig:pipeline}:
(1)~\emph{ingestion}, which pulls audio and collects YouTube chapter
...
features; (4)~\emph{boundary prediction and selection}, which runs
...
Several design decisions that might look like limitations were made
deliberately, and each is worth a brief explanation.

Thirty videos is small by contemporary NLP standards. The choice was
intentional: a 30-video benchmark lets every prediction be traced to
its source video, every metric value verified against a reproducible
reference, and every failure inspected directly. Large-scale benchmarks
are better suited to measuring aggregate performance; this one is designed
...
redistribution of raw video, and reflects the navigation structure actual
...
would undermine the benchmark's role as a pure evaluation target. Keeping
segmentation unsupervised and making the method selector the only supervised
...
boundary two sentences early as severely as a completely missing boundary,
which misrepresents what actually matters when a student is trying to navigate
a lecture recording.
...
Selection criteria were: (1) duration at least 15 minutes; (2) clean monolingual
English audio; (3) creator-supplied YouTube chapter markers with at least 4
distinct chapters; and (4) openly licensed content.
The resulting corpus covers 32.52~hours of audio, with 419 chapter
...
videos --- this is reported as the manifest state rather than as a
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/annotation_tool_main.png}
  \caption{The Streamlit annotation interface. Annotators step through the
    transcript sentence-by-sentence, with creator chapter boundaries already
    marked (shown as yellow dividers). Clicking a sentence places a subtopic
    boundary marker at that position.}
  \label{fig:annotation_tool_main}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/annotation_tool_subtopics.png}
  \caption{The annotation tool showing subtopic boundary placement within a
    chapter. Placed boundaries appear as green markers. Annotators could
    add, remove, or adjust markers before saving the session.}
  \label{fig:annotation_tool_subtopics}
\end{figure}
...
this range; no chapter with more than five genuinely distinct sub-ideas was
encountered, though the guideline would not have prevented additional marks
in such a case.
...
This is the most important conceptual limitation of \lecseg{} and it needs to be
stated precisely: the benchmark measures a system's ability to reproduce
\emph{creator navigation metadata}, which is not the same thing as discovering
...
A system with a low $P_k$ is correctly predicting the structure of
creator-annotated navigation metadata.
A system with a high $P_k$ may still produce educationally superior
...
The choice to use creator chapters is nonetheless defensible.
Creator chapters are a real-world, audience-facing artifact: they represent
what actual viewers see when navigating YouTube lecture videos.
...
The benchmark is best understood as measuring \emph{creator-aligned
...
so agreement here mainly reflects that both passes observe the same
...
it characterises genuine human agreement on the finer-grained task
where annotator judgement is actually required.
...
discourse-segmentation task where reasonable annotators can disagree about
whether a rhetorical transition merits a boundary mark.
This is documented as a threat to annotation validity in
...
preceding the sentence (gaps of at least 300~ms were counted as significant);
...
This entropy weighting is a practical confidence proxy, not a proof of true
modality reliability. A noisy modality can also produce sharp peaks. For this
reason, Chapter~\ref{chap:results} reports fusion and modality ablations
separately and demotes modalities that hurt chapter-level $P_k$/WindowDiff.
...
This design separates recall (text-only pass) from precision (multimodal
refinement pass), avoiding the sensitivity of pure threshold methods to
per-video score shifts.
...
boundary set.
The experimental results do not support treating LLM refinement as the
main source of metric gain; it is retained as an implemented diagnostic
...
Different configurations work best on different videos; no single
...
pool. Expanding to 120 methods degrades performance, suggesting that pool
...
Under a leave-domain-out evaluation, where the held-out video's entire academic
domain is excluded from training, the selector degrades to
...
and three per-fold training examples are not enough to provide reliable
domain-specific guidance.
The selector should therefore be read as a low-resource benchmark
...
Both are window-based error rates, meaning they measure how often the
...
This choice directly matches the target application: a student navigating
a recorded lecture can tolerate a boundary that is a few seconds early or late.
Penalising near-misses as harshly as complete misses, as exact-match metrics
do, would over-penalise practically acceptable errors.
...
TreeSeg) on published benchmarks. No single metric is perfect: $P_k$ is
sensitive to near-boundary placement; WindowDiff does not account for partial
segment overlap. Their complementary failure modes are why both are reported.
...
\emph{how} a method fails; none of them rank methods or drive the primary claims.
...
it reflects a different operating objective than $P_k$/WD, and is reported
only to characterise the operating point.
...
deployment-realistic assumption because a live system generally does not know
the correct number of chapters in advance. Chapter~\ref{chap:results}
