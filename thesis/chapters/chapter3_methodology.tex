...
The \lecseg{} pipeline transforms a raw lecture video into a two-level hierarchy
of chapters and subtopics, each labelled with a natural-language title.
It comprises five stages, depicted in Figure~\ref{fig:pipeline}:
(1)~\emph{ingestion}, which extracts audio and collects YouTube chapter metadata;
(2)~\emph{preprocessing}, which produces time-aligned sentences from the
speech signal and auxiliary visual and prosodic features;
(3)~\emph{feature extraction}, which maps each sentence to a multimodal
embedding vector;
(4)~\emph{boundary prediction}, which applies the novel two-stage predictor
with reliability-weighted fusion to locate chapter and subtopic boundaries; and
(5)~\emph{LLM refinement}, which uses a local large language model to adjust
boundary positions by up to $\pm 2$ sentences and assign titles to each segment.

\begin{figure}[t]
  \centering
  % \includegraphics[width=\linewidth]{figures/pipeline.pdf}
  \fbox{\parbox{0.95\linewidth}{\centering Figure placeholder --- pipeline diagram\\[4pt]
    \textbf{Video} $\xrightarrow{\text{ASR}}$ \textbf{Sentences}
    $\xrightarrow{\text{Embed}}$ \textbf{Vectors}
    $\xrightarrow{\text{RWF}}$ \textbf{Fused signal}
    $\xrightarrow{\text{TwoStage}}$ \textbf{Boundaries}
...
The resulting corpus comprises approximately 55~hours of audio (mean video
duration $109 \pm 138$~min; median $57$~min), with a total of 329 chapters
(mean $11 \pm 4$ per video).
...
metadata as a reproducible, creator-validated reference.
...
tolerance.
Results are reported in Section~\ref{sec:results_iaa}.
...
using the \texttt{all-mpnet-base-v2} model (768-dim) as the primary encoder.
We additionally evaluated \texttt{all-MiniLM-L6-v2} (SBERT), \texttt{intfloat/e5-base-v2}
and \texttt{BAAI/bge-base-en-v1.5} as ablation variants.
...
\section{Local-LLM refinement and titling (N4)}
\label{sec:refine}
...
If Ollama is unavailable the refinement step is silently skipped.
...
Methods requiring $n_{\text{segments}}$ receive the ground-truth count at
inference time, consistent with standard practice~\citep{choi2000latent}.
We also report numbers for our automatic segment-count estimator in
Appendix~\ref{app:extra_results}.
