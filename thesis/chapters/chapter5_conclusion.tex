...
A four-video external spot-check (Section~\ref{sec:external_validation})
found BGE-divisive to be consistent (external mean $P_k=0.398$ vs
$0.388$ on \dataset{}) but found the cross-model conservative method to
degrade more substantially ($P_k=0.461$ external vs $0.371$), suggesting it
is partly calibrated to LecSeg-30's Whisper transcription style and
sentence-density distribution.
These four videos use yt-dlp auto-captions rather than Whisper
transcripts, so caption-quality differences are an additional confound.
